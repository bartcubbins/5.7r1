			...
			u8 *val, int count)
{
	...
	if (rc)
		pr_err("Read failed base=0x%02x sid=0x%02x rc=%d\n",
				...
				u16 offset, u8 *val, int count)
{
	...
	if (rc) {
		pr_err("Read failed base=0x%02x sid=0x%02x rc=%d\n",
				base + SEC_ACCESS, to_spmi_device(pdev->dev.parent)->usid, rc);
		return rc;
	}

	rc = __qpnp_lbc_write(chip, base + offset, val, 1);
	if (rc)
		pr_err("Write failed base=0x%02x sid=0x%02x rc=%d\n",
				base + SEC_ACCESS, to_spmi_device(pdev->dev.parent)->usid, rc);
				...
				u16 offset, u8 mask, u8 val)
{
	...
	if (rc) {
		pr_err("write failed base=0x%02x sid=0x%02x rc=%d\n",
				base + offset, to_spmi_device(pdev->dev.parent)->usid, rc);
		return rc;
	}
...
static int qpnp_lbc_chg_init(struct qpnp_lbc_chip *chip)
{
	...
	if (of_find_property(chip->pdev->dev.of_node, "qcom,tchg-mins", NULL)) {
...
static int qpnp_charger_read_dt_props(struct qpnp_lbc_chip *chip)
{
	...
	chip->cfg_btc_disabled = of_property_read_bool(
			chip->pdev->dev.of_node, "qcom,btc-disabled");
	...
	chip->cfg_use_external_charger = of_property_read_bool(
			chip->pdev->dev.of_node, "qcom,use-external-charger");
	...
	if (of_find_property(chip->pdev->dev.of_node,
		...
		rc = of_property_read_u32_array(chip->pdev->dev.of_node,
				"qcom,thermal-mitigation",
...
static int qpnp_lbc_request_irq(struct qpnp_lbc_chip *chip,
...
static int qpnp_lbc_request_irqs(struct qpnp_lbc_chip *chip)
{
	...
	for_each_available_child_of_node(chip->pdev->dev.of_node, child) {
		...
		case LBC_CHGR_SUBTYPE:
			rc = qpnp_lbc_request_irq(chip, child, chip->chg_error_irq,
				"chg-failed", qpnp_lbc_chg_failed_irq_handler,
				(IRQF_TRIGGER_RISING));
			if (rc < 0)
				return rc;
			rc = qpnp_lbc_request_irq(chip, child, chip->fastchg_irq,
				"fast-chg-on", qpnp_lbc_fastchg_irq_handler,
				(IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT));
			if (rc < 0)
				return rc;
			rc = qpnp_lbc_request_irq(chip, child, chip->vbatdet_lo_irq,
				"vbat-det-lo", qpnp_lbc_vbatdet_lo_irq_handler,
				(IRQF_TRIGGER_FALLING));
			if (rc < 0)
				return rc;
			rc = qpnp_lbc_request_irq(chip, child, chip->chg_done_irq,
				"chg-done",  qpnp_lbc_chg_done_irq_handler,
				(IRQF_TRIGGER_RISING));
			if (rc < 0)
				return rc;
			enable_irq_wake(chip->chg_error_irq);
			enable_irq_wake(chip->fastchg_irq);
			enable_irq_wake(chip->vbatdet_lo_irq);
			enable_irq_wake(chip->chg_done_irq);
			break;
		case LBC_BAT_IF_SUBTYPE:
			rc = qpnp_lbc_request_irq(chip, child, chip->batt_pres_irq,
				"batt-pres", qpnp_lbc_batt_pres_irq_handler,
				(IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT));
			if (rc < 0)
				return rc;
			rc = qpnp_lbc_request_irq(chip, child, chip->batt_temp_irq,
				"bat-temp-ok", qpnp_lbc_batt_temp_irq_handler,
				(IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING));
			if (rc < 0)
				return rc;
			enable_irq_wake(chip->batt_pres_irq);
			enable_irq_wake(chip->batt_temp_irq);
			break;
		case LBC_USB_PTH_SUBTYPE:
			rc = qpnp_lbc_request_irq(chip, child, chip->usbin_valid_irq,
				"usbin-valid", qpnp_lbc_usbin_valid_irq_handler,
				(IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING));
			if (rc < 0)
				return rc;
			rc = qpnp_lbc_request_irq(chip, child, chip->usb_overtemp_irq,
				"usb-over-temp", qpnp_lbc_usb_overtemp_irq_handler,
				(IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING));
			if (rc < 0)
				return rc;
			rc = qpnp_lbc_request_irq(chip, child, chip->chg_gone_irq,
				"chg-gone", qpnp_lbc_chg_gone_irq_handler,
				(IRQF_TRIGGER_RISING));
			if (rc < 0)
				return rc;
			enable_irq_wake(chip->usbin_valid_irq);
			enable_irq_wake(chip->usb_overtemp_irq);
...
static int qpnp_lbc_parallel_probe(struct platform_device *pdev)
{
	...
	chip->parallel_psy_d.property_is_writeable = qpnp_lbc_parallel_is_writeable;
	chip->parallel_psy_d.num_properties = ARRAY_SIZE(qpnp_lbc_parallel_properties);
...
static int qpnp_lbc_main_probe(struct platform_device *pdev)
{
	...
	if (chip->bat_if_base) {
		...
		chip->batt_psy_d.num_properties = ARRAY_SIZE(msm_batt_power_props);
		chip->batt_psy_d.get_property = qpnp_batt_power_get_property;
		chip->batt_psy_d.set_property = qpnp_batt_power_set_property;
		chip->batt_psy_d.property_is_writeable = qpnp_batt_property_is_writeable;
		chip->batt_psy_d.external_power_changed = qpnp_batt_external_power_changed;
...
/*
 * qpnp_lbc_init() - register driver for qpnp-chg
 */
