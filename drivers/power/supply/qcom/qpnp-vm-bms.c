...
do {									\
	chip->irq_name##_irq.irq = platform_get_irq_byname(pdev, /* TODO: check chip->pdev!!! */	\
						#irq_name);		\
...
static int bms_find_irqs(struct qpnp_bms_chip *chip, struct platform_device *pdev)
{
	int rc = 0;

	rc = platform_get_irq_byname(pdev, "fifo_update_done");
	if (rc < 0)
		return rc;
	
	rc = platform_get_irq_byname(pdev, "fsm_state_change");
	if (rc < 0)
		return rc;
...
static int parse_spmi_dt_properties(struct qpnp_bms_chip *chip, struct platform_device *pdev)
{
	//struct platform_device *pdev = chip->pdev;
	...
	for_each_available_child_of_node(pdev->dev.of_node, child) {
		...
		if (strcmp("qcom,batt-pres-status", child->name) == 0) {
			chip->batt_pres_addr = base;
			continue;
		}

		if (strcmp("qcom,qpnp-chg-pres", child->name) == 0) {
			chip->chg_pres_addr = base;
			continue;
		}

		chip->base = base;
		rc = bms_find_irqs(chip, pdev);
		if (rc) {
...
static int bms_get_adc(struct qpnp_bms_chip *chip, struct platform_device *pdev)
{
	//struct platform_device *pdev = chip->pdev;
	int rc = 0;
...
static int qpnp_vm_bms_probe(struct platform_device *pdev)
{
	...
	rc = bms_get_adc(chip, pdev); //TODO: Check this or (chip, pdev)
	if (rc < 0) {
	...
	rc = parse_spmi_dt_properties(chip, pdev); //TODO: Check this or (chip, pdev)
	if (rc) {
	...
	chip->bms_psy_d.property_is_writeable = qpnp_vm_bms_property_is_writeable;
...
{
	return platform_driver_unregister(&qpnp_vm_bms_driver);
}
